...
void lineModMatch::scale_test(string mode, Point left_up_point, int num_feature, int retain_nums, float low_scale_value,
							  float high_scale_value, float scale_step)
{
	...
	if (mode == "train") {
		...
		for (size_t i = 0; i<top5; i++) {
			...
			left_up_point = Point(match.x, match.y);
		}
...
void lineModMatch::angle_test(string mode, Point left_up_point, bool use_rot, int num_feature, int retain_nums, float low_angle_value,
...
void lineModMatch::noise_test(string mode, Point left_up_point, int num_feature, float low_angle_value, float high_angle_value,
							  float angle_step, float low_scale_range, float high_scale_range, float scale_step)
{
	...
	if (mode == "train") {
		...
		for (auto idx : idxs) {
			...
			cv::circle(test_img, { match.x, match.y }, 3, Scalar(255, 0, 0), -1);
