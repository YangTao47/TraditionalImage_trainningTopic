...
public:
	...
	void scale_test(string mode = "test", Point left_up_point = Point(0, 0), int num_feature = 38, int retain_nums = 5, float low_scale_value = 0.1f,
					float high_scale_value = 1, float scale_step = 0.01f);
	/*多角度、单尺度模板训练匹配（一张图内智能匹配单个目标）use_rot是否直接使用旋转初始图特征点*/
	void angle_test(string mode = "test", Point left_up_point = Point(0,0), bool use_rot = true, int num_feature = 20, int retain_nums = 1,
					float low_angle_value = 0, float high_angle_value = 360, float angle_step = 1, float scale_range = 1);
	/*多角度、多尺度带噪声模板训练匹配（一张图内可以匹配多个目标）*/
	void noise_test(string mode = "test", Point left_up_point = Point(0, 0), int num_feature = 38, float low_angle_value = 0,
