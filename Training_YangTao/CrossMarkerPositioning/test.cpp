...
static std::string prefix = "D:/linemod/linemod/test/";
...
void scale_test(string mode = "test") {
	...
	if (mode == "train") {
		...
		Mat test_img = imread(prefix + "case0/2.jpg");
...
void angle_test(string mode = "test", bool use_rot = true) 
{
	...
	if (mode != "test") {
		Mat img = imread(prefix + "case3/train.jpg");
		assert(!img.empty() && "check your img path");
		...
		detector.writeClasses(prefix + "case3/%s_templ.yaml");
		shapes.save_infos(infos_have_templ, prefix + "case3/test_info.yaml");
		std::cout << "train end" << std::endl << std::endl;
	}
	else if (mode == "test") {
		std::vector<std::string> ids;
		...
		auto infos = shape_based_matching::shapeInfo_producer::load_infos(prefix + "case3/test_info.yaml");

		Mat test_img = imread(prefix + "case3/1.bmp");
		assert(!test_img.empty() && "check your img path");
		...
		for (size_t i = 0; i<top5; i++) 
		{
			...
			cout << templ[0].tl_x << endl;
			cout << templ[0].tl_y << endl;
			circle(Img, { (int)x, (int)y }, 1, {255,0,0}, -1);
			circle(Img, { (int)match.x, (int)match.y }, 1, { 0,255,0 }, -1);
			circle(Img, { (int)templ[0].tl_x, (int)templ[0].tl_y }, 10, { 0,0,0 }, -1);
			...
			for (int i = 0; i<templ[0].features.size(); i++) {
				auto feat = templ[0].features[i];
				Point p;
				cv::circle(Img, { feat.x + match.x, feat.y + match.y }, 1, randColor, -1);
				p.x = feat.x + match.x;
				p.y = feat.y + match.y;
				P.push_back(p);
			}
...
void noise_test(string mode = "test") {
	...
	if (mode == "train") {
		...
		//Mat test_img = imread(prefix+"case3/1.bmp");
		Mat test_img = imread("产品/偏右旋/2.bmp");
